...
const BagSummary = () => {
  ...
  const totalAmount = cartItems.reduce((total, item) => total + item.price, 0); // Calculate total price
        ...
        <div className="row">
          {cartItems.map((item, index) => (
            <div key={index} className="col-6 col-sm-4 mb-3">
              <img src={item.image} className="img-thumbnail" alt={item.name} style={{ width: '100%', height: '100%', borderRadius: '12px' }} />
            </div>
          ))}
        </div>
      </div>
